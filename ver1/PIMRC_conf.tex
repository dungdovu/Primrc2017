...
\title{A Waste City Management System for Smart Cities Applications }
...
\begin{table}[]
	...
	\begin{tabular}{|l|l|}
		...
		$S=\{s_{i} | i \in (1,n) \}$	& \begin{tabular}[c]{@{}l@{}}Status of each bin   $i$   \end{tabular}       \\ \hline
		$L=\{l_{i} | i \in (1,n) \}$	& \begin{tabular}[c]{@{}l@{}}Location of each bin   $i$   \end{tabular}       \\ \hline
		$W=\{w_i | i \in (1,n)\}$	& Weight of each bin $i$            \\ \hline
		$T=\{t_{di} | d \in (1,m); i \in (1,n) \}$	& \begin{tabular}[c]{@{}l@{}}Collected data milestone of\\ each bin     $i$ \end{tabular}       \\ \hline
		
		$\mathcal{D}=\{S,L,W, T\}$	&Input data         \\ \hline
		
		
		$\mathcal{M}=\{M_j | j \in (1,k)\}$	& 
		
		\begin{tabular}[c]{@{}l@{}}Optimal route of each \\garbage truck within each \\cluster $M_j$\end{tabular}
		\\ \hline
		\begin{tabular}[c]{@{}l@{}}
			$\mathcal{DC}=\{DC_j | j \in (1,k)$; \\$DC_j = \{S_j, L_j, W_j, T_j\} \}$	\end{tabular}& 
		
		\begin{tabular}[c]{@{}l@{}}Working cluster\end{tabular}
		\\ \hline
		
		$k$	& Number of working clusters         \\ \hline
		
		$\eta$	& Threshold of amount waste       \\ \hline
		
		$n$	& \begin{tabular}[c]{@{}l@{}}Number of trash bins in the \\dataset \end{tabular}    \\ \hline
		
		$m()$	& \begin{tabular}[c]{@{}l@{}}Dynamic vector of number \\trash bins in each working \\cluster\end{tabular}    \\ \hline
...
In this section, using the proposed Algorithm \ref{algorithm1} in Section \ref{systemdesciption}, we first analyze the number of working cluster and then show the experimental result. We simply assume $\eta = 0.5$, which can also be set to another value to monitor the amount of waste. 
