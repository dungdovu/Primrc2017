...
However, we observed that citizens in the city tend to throw their waste into the bins without a particular time of a day and the manager need to add more trash bins during the holiday event time. But these previous works do not solve this issue. Therefore, a method of automatically classifying the location trash bin, predicting the status of each one, and optimizing the garbage truck routes will help the city's waste management system to operate more efficiently. 
...
\end{itemize}
\par This paper is organized as the follows: In section 2,  a new method of smart waste city management (SWCM) is explained and the structure of the algorithm is proposed. Section 3 presents the SWCM simulation model. Finally, the related concluding remarks are discussed in section 4.
