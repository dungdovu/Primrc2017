...
\par  The overall procedure is summarized in Algorithm \ref{algorithm1}. In this algorithm, the parameters are defined in Table \ref{table1}. We suppose that the input dataset $\mathcal{D}$ is the database in Section II. The parameter $\eta$, $n$, $t$ and $k$ denotes the threshold of amount waste in each bin, the number of bins, the time stamp of collecting data, and the number of working clusters, respectively.
